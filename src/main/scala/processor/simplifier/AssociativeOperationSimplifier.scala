...
abstract class AssociativeOperationSimplifier {
  ...
  private def simplifyAssociativeOperation(expression: FunEqExpression): FunEqExpression = {
    ...
    aggregate(operation, processed)
  }

  // Combine every pair of operands together
  // For example: (2, x, 4) -> (x, 6)
  protected def processOperands(operands: List[FunEqExpression]): List[FunEqExpression] = {
    operands match {
  ...
  private def combineTwoExtended(x: FunEqExpression, y: FunEqExpression): Option[FunEqExpression] = {
    (x, y) match {
